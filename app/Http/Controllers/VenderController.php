...
class VenderController extends Controller
{
    ...
    public function terminarVenta(Request $request)
    {
        ...
        foreach ($productos as $producto) {
            ...
            (String) $idupdate = $productoActualizado->id;
            $productoActualizado->existencia -= $productoVendido->cantidad;
            $cliente = Http::put('https://APIFacturacionMongoDB.anthony-patrici.repl.co/products/'.$idupdate , $productoActualizado
            );
    ...
    public function actualizarProductoDeVenta(Request $request)
    {
        $indice = $request->post("indice");
        $productos = $this->obtenerProductos();
        if($productos[$indice]->existencia < $request->post("cantidad") ){
            return redirect()->route("vender.index")
                ->with([
                    "mensaje" => "No existen suficientes existencias, solo hay disponibles ".$productos[$indice]->existencia." productos a disposición",
                    "tipo" => "danger"
                ]);
        }else if($request->post("cantidad")<=0){
            return redirect()->route("vender.index")
            ->with([
                "mensaje" => "Error al actualizar la cantidad, ingrese un valor positivo diferente de 0.",
                "tipo" => "danger"
            ]);
        }
        else if($productos[$indice]->existencia >= $request->post("cantidad")){
            $productos[$indice]->cantidad=$request->post("cantidad");
            return redirect()
            ->route("vender.index")
