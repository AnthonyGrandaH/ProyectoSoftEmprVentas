...
Route::group(['prefix' => 'auth'], function () {
    ...
    Route::group(['middleware' => 'auth:api'], function() {
        ...
        Route::put("/cliente", function(Request $request){
            $cliente = Cliente::findOrFail($request->input("cedula"));
            $cliente->fill($request->input());
            $cliente->saveOrFail();
            return response()->json(true);
        });
        Route::delete("/cliente/{cedula}", function($cedula){
            $cliente = Cliente::findOrFail($cedula);
            $cliente->delete();
            return response()->json(true);
        });
        ...
        Route::get('/products', function(){
            $client = new Client([
                'base_uri' => 'https://APIFacturacionMongoDB.anthony-patrici.repl.co/',
            ]);
        ...
        Route::put("/products/{id}", function(Request $request){
            $client = new Client([
                'base_uri' => 'https://APIFacturacionMongoDB.anthony-patrici.repl.co/',
            ]);
